...
from pymor.discretizers.elliptic import discretize_elliptic_cg, discretize_elliptic_cg_ndim, discretize_elliptic_fv
...
from pymor.algorithms.timestepping import ImplicitEulerTimeStepper, ExplicitEulerTimeStepperNDim
...
def discretize_parabolic_cg_ndim(analytical_problem, sysdim, diameter=None, domain_discretizer=None,
                           grid=None, boundary_info=None, num_values=None, time_stepper=None, nt=None):
    """Discretizes an |ParabolicProblem| using finite elements.
    ...
    d, data = discretize_elliptic_cg_ndim(p.elliptic_part(), sysdim, diameter=diameter, domain_discretizer=domain_discretizer,
        grid=grid, boundary_info=boundary_info)
    ...
    for j in range(sysdim):
        mu['komp'] = j
        I[j] = p.initial_data.evaluate(data['grid'].centers(data['grid'].dim),mu)
    ...
    discretization = InstationaryDiscretizationNDim(sysdim=sysdim, operator=d.operator, rhs=d.rhs, mass=mass, initial_data=I, T=p.T,
                                                products=d.products, time_stepper=time_stepper,
                                                parameter_space=d.parameter_space, visualizer=d.visualizer,
                                                num_values=num_values, name='{}_CG'.format(p.name))
