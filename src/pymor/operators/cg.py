...
class L2ProductP1Absorb(NumpyMatrixBasedOperator):
    '''|Operator| representing the L2-product between linear finite element functions and an absorbtion function c(x)

    To evaluate the product use the :meth:`~pymor.operators.interfaces module.OperatorInterface.apply2`
    method.

    (c(x)u(x),v(x)) with a given function c(x)
    ...
    def __init__(self, grid, boundary_info, absorb_function=None, dirichlet_clear_rows=True, dirichlet_clear_columns=False,
        ...
        self.dirichlet_clear_diag = dirichlet_clear_diag
        self.name = name
        if absorb_function is not None:
    ...
    def _assemble(self, mu=None):
        ...
        self.logger.info('Integrate the products of the shape functions on each element')
        # -> shape = (g.size(0), number of shape functions ** 2)
        if self.absorb_function is not None:
            A=self.absorb_function(self.grid.centers(0),mu=mu)
            SF_INTS = np.einsum('iq,jq,q,e,e->eij', SFQ, SFQ, w, g.integration_elements(0),A).ravel()
        else:
            SF_INTS = np.einsum('iq,jq,q,e->eij', SFQ, SFQ, w, g.integration_elements(0)).ravel()
