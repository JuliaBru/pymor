...
class InstationaryDiscretizationNDim(DiscretizationBase):
    ...
    def __init__(self, sysdim, T, initial_data=None, operator=None, rhs=None, mass=None, time_stepper=None, num_values=None,
                 products=None, operators=None, functionals=None, vector_operators=None, parameter_space=None,
                 estimator=None, visualizer=None, cache_region=None, name=None):
        functionals = functionals or {}
        operators = operators or {}
        ...
        for j in range(sysdim):
            if isinstance(initial_data[j], VectorArrayInterface):
                initial_data[j] = VectorOperator(initial_data[j], name='initial_data[j]')
            assert isinstance(initial_data[j], OperatorInterface)
            assert initial_data[j].source == NumpyVectorSpace(1)
        assert 'initial_data' not in vector_operators or initial_data == vector_operators['initial_data']
        ...
        super(InstationaryDiscretizationNDim, self).__init__(operators=operators_with_operator_mass,
                                                             functionals=functionals_with_rhs,
                                                             vector_operators=vector_operators_with_initial_data,
                                                             products=products, estimator=estimator,
                                                             visualizer=visualizer, cache_region=cache_region, name=name)
