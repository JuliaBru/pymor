...
def greedy_fp(MaxOrdn,imax,sample,seed=None):
    ...
    for m_ind in range(MaxOrdn):
        ...
        for mu in problem.parameter_space.sample_randomly(sample):
            ...
            V[sample_ind],discr=fp_demo(args,(Basis[sample_ind],discretization))
        ...
        for i_ind in range(1,imax):
            ...
            #Bestimme Parameter, in deren Umgebung verfeinert werden soll
            indices=range(relerror.shape[0])
            mukern=dict.fromkeys(range(n_mukern))
            mukern[0]=mudict[snapshot_min_ind]
            indices.remove(snapshot_min_ind)
            eps=1.
            for jind in range(1,n_mukern):
                while True:
                    next_min=np.ma.argmin(relerror[indices])
                    indices.remove(next_min)
                    if dg(mudict[snapshot_min_ind],mudict[next_min]) >= eps:
                        mukern[j]=mudict[next_min]
                        break

            #Suche Parameter in der Naehe des besten Parameters aus erster Iteration
            for sample_ind in range(i_ind*sample, int((i_ind+1./2.)*sample)):
            ...
            for sample_ind in range(int((i_ind+1./2.)*sample), (i_ind+1)*sample):
        ...
        while True:
            try:
                Vend,discr=fp_demo(args,(Basis[np.argmin(relerror)],discretization))
                break
            except ValueError:
                args['--CFL']*=0.75
                print('neue CFL={}'.format(args['--CFL']))
