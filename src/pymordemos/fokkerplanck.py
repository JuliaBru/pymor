...
"""
Solves reduced system derived from Fokker-Planck equation
"""
...
def fp_system(m, test_case='SourceBeam', n_grid=500,
    ...
    assert (basis_type == 'Leg' and basis_pl_discr is None) or (basis_type == 'RB' and basis_pl_discr is not None)
    assert CFL_type in ('Auto', 'Manual')
    assert (CFL_type == 'Auto' and CFL is None) or (CFL_type == 'Manual' and CFL is not None)
